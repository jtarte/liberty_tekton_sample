...
The pipepline will update the application deployment file with the new version and the new image generated by the CI pipelines. the update of the page will be detected by ArgoCD that will deploy the new version of the application. 

To update the git content, the demo used a custom `Task` that change the [kube/application.yaml file](../kube/application.yaml) by updating the version and the image fields. 
...
The task prepare the operation with git by setting the config. Then it change the version and image tag with the new value using a sed command. And finally, it pushed the updated file to git. The authentication with Github is handled by the service account that runs the pipeline. 
...
Another option could be to used the `liberty-build-deploy-pipeline pipeline`, decribed [here](./cicd-tekton.md). but instead to deploy the application directly, you could change the script in the `PipelineRun` to use the  `./gitops/application.yaml` script. 
